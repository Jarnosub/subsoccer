...
# 2. Upload this script, 'subsoccer_new_cities_daily.csv', and 'subsoccer_new_cities_cumulative.csv'
# 3. Create a cell and run:
#    !git clone https://github.com/google-research/timesfm.git
#    !pip install ./timesfm[torch]
# 4. Run this script: !python timesfm_new_cities_forecast.py
#
# Option B: Run Locally
# 1. Install prerequisites:
#    pip install matplotlib pandas numpy
#    pip install timesfm (or install from git: pip install git+https://github.com/google-research/timesfm.git)
# 2. Run: python timesfm_new_cities_forecast.py
# -------------------------------------------------------------

def run_new_cities_forecast():
    daily_csv = 'subsoccer_new_cities_daily.csv'
    cumulative_csv = 'subsoccer_new_cities_cumulative.csv'
    
    # Resolve paths
    base_dir = os.path.dirname(__file__)
    if not os.path.exists(daily_csv):
        daily_csv = os.path.join(base_dir, 'subsoccer_new_cities_daily.csv')
    if not os.path.exists(cumulative_csv):
        cumulative_csv = os.path.join(base_dir, 'subsoccer_new_cities_cumulative.csv')
        
    if not os.path.exists(daily_csv) or not os.path.exists(cumulative_csv):
        print("Error: Could not find new cities CSV files. Make sure they are in the same directory.")
        return
    ...
    print(f"Loaded {len(y_daily)} days of new cities telemetry data.")
    print(f"Current Total Unique Cities: {int(y_cumulative[-1])}")
    ...
    # 3. Forecast Daily New Cities (Rate)
    horizon = 14
    if has_timesfm:
        ...
        # Since cumulative can't decrease, enforce monotonic growth starting from last value
        ...
        pred_daily = [max(0.0, mean_rate + np.random.normal(0, 0.4)) for _ in range(horizon)]
        
        # Cumulative projection: just sum the predicted daily additions onto the last cumulative count
        pred_cumulative = []
    ...
    # 4. Plot Chart 1: Daily New Cities Rate
    plot_results(
        dates_daily, y_daily, forecast_dates, pred_daily,
        "Daily New Cities Addition Rate", "New Cities / Day",
        "subsoccer_new_cities_rate_forecast.png", "#34a853"
    )
    
    # 5. Plot Chart 2: Cumulative Cities Growth
    plot_results(
        dates_cumulative, y_cumulative, forecast_dates, pred_cumulative,
        "Cumulative Unique Cities Growth Forecast", "Total Unique Cities",
        "subsoccer_new_cities_cumulative_forecast.png", "#fbbc05"
    )
