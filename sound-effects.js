...
class SoundEffects {
    ...
    /**
     * Play looping background gameplay music
     */
    playGameplayTheme() {
        if (!this.enabled) return;
        this.stopMusic(); // Stop any currently playing background music

        if (this.sounds['gameplay']) {
            this.currentMusic = this.sounds['gameplay'];
            this.currentMusic.loop = true;
            this.currentMusic.currentTime = 0;
            this.currentMusic.volume = this.volume * 0.5; // BG music should be slightly quieter
            this.currentMusic.play().catch(e => console.log('Gameplay theme play failed:', e));
        }
    }
    ...
    playVictoryTheme() {
        ...
        if (this.sounds['victory']) {
            this.currentMusic = this.sounds['victory'];
            this.currentMusic.loop = false;
            this.currentMusic.currentTime = 0;
            this.currentMusic.volume = this.volume * 0.8;
            this.currentMusic.play().catch(e => console.log('Victory theme play failed:', e));
        }
    }

    /**
     * Stop currently playing background or victory music
     */
    stopMusic() {
