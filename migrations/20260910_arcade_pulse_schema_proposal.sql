...
-- Yhdistää fyysisen tai simuloidun kytkimen olemassa olevaan games.id-tunnisteeseen.
CREATE TABLE IF NOT EXISTS public.arcade_table_configs (
    table_id UUID PRIMARY KEY REFERENCES public.games(id) ON DELETE CASCADE,
    is_enabled BOOLEAN NOT NULL DEFAULT true,
    switch_type TEXT NOT NULL DEFAULT 'simulation', -- 'simulation', 'netio_cloud', 'shelly_cloud', 'custom_rest'
    ...
    switch_output_id INT NOT NULL DEFAULT 1,       -- releen/pistorasian numero (oletus 1)
...
CREATE TABLE IF NOT EXISTS public.arcade_sessions (
    ...
    table_id UUID NOT NULL REFERENCES public.games(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS public.arcade_events (
    ...
    table_id UUID NOT NULL REFERENCES public.games(id) ON DELETE CASCADE,
    session_id UUID REFERENCES public.arcade_sessions(id) ON DELETE SET NULL,
    event_type TEXT NOT NULL, 
    -- 'session_requested', 'switch_cmd_sent', 'switch_confirmed_on', 
    -- 'session_expired', 'switch_confirmed_off', 'force_stopped', 'switch_error'
