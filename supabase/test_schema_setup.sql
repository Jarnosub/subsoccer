-- ==============================================================================
-- SUBSOCCER ARCADE — TEST DATABASE SETUP SCRIPT
-- Consolidated setup for new/blank Supabase test project.
-- Run this entire script in Supabase SQL Editor.
-- ==============================================================================

-- 1. BASE ARCADE TABLES
CREATE TABLE IF NOT EXISTS public.arcade_table_configs (
    table_id TEXT PRIMARY KEY,
    game_id UUID,
    is_enabled BOOLEAN NOT NULL DEFAULT true,
    is_free_play_allowed BOOLEAN NOT NULL DEFAULT false,
    switch_type TEXT NOT NULL DEFAULT 'netio_json',
    switch_endpoint TEXT,
    switch_auth_secret TEXT,
    switch_output_id INT NOT NULL DEFAULT 1,
    default_duration_seconds INT NOT NULL DEFAULT 900,
    max_duration_seconds INT NOT NULL DEFAULT 3600,
    lock_state TEXT NOT NULL DEFAULT 'available' CHECK (lock_state IN ('available', 'pending_payment', 'active', 'maintenance_locked', 'error_locked')),
    notes TEXT,
...
CREATE TABLE IF NOT EXISTS public.arcade_sessions (
    ...
    auth_source TEXT NOT NULL DEFAULT 'stripe' CHECK (auth_source IN ('free_play', 'test_table', 'stripe', 'admin')),
...
-- 5. Oikeuksien hallinta kaikille 7 RPC-funktiolle
