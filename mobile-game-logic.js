...
window.mobileAddPlayer = function(defaultName, userId) {
    ...
    div.innerHTML = `
        ...
        <input type="text" autocomplete="off" autocapitalize="characters" onfocus="setTimeout(() => this.select(), 50)" onkeyup="this.setAttribute('value', this.value); if(window.broadcastTvState) window.broadcastTvState();" value="${defaultName || ('PLAYER ' + num)}" 
               ...
               style="text-transform: uppercase; color: white; width: 100%; padding: 8px 10px; font-size: 1rem; font-weight: 700; background: rgba(255,255,255,0.05); border: 1px dashed rgba(255,255,255,0.2); border-radius: 6px; outline: none; font-family: 'Resolve', sans-serif; letter-spacing: 1px;">
